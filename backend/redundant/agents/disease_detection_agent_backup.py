...
class DiseaseDetectionAgent(ToolAgent):
    ...
    async def run(self, state: AgentState) -> AgentState:
        """
        Execute disease detection workflow

        Workflow:
        1. Validate images are present in state
        2. Decode base64 image to bytes
        3. Extract crop_type from user_query using LLM (if query exists and LLM available)
        4. Execute disease detection tool
        ...
        Returns:
            Updated state with disease_output containing:
                - If successful: disease_name, crop, treatment, severity, etc.
        ...
        try:
            ...
            if not images or len(images) == 0:
                error_msg = f"{self.name}: No images provided. Disease detection requires at least one image."
                logger.warning(error_msg)
                state["disease_output"] = {
                    "error": error_msg,
            ...
            # Phase 2: Get base64 image (PyTorch tool handles decoding internally)
            ...
            if image_base64.startswith('data:'):
                if ';base64,' in image_base64:
                    ...
                    error_msg = f"{self.name}: Invalid data URI format"
            ...
            logger.info(f"{self.name}: Image base64 string ready ({len(image_base64)} chars)")
            ...
            # Phase 4: Execute disease detection tool
            # PyTorch tool expects base64 string, not bytes
            # Use .invoke() for LangChain tools (not deprecated __call__)
            ...
            # Phase 5: Update state with results
            if "error" in result:
                logger.warning(f"{self.name}: Tool execution returned error: {result['error']}")
                state["disease_output"] = result
            else:
                logger.info(
