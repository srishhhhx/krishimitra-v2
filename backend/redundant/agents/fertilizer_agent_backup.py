...
class FertilizerAgent(ToolAgent):
    ...
    async def run(self, state: AgentState) -> AgentState:
        """
        Execute fertilizer recommendation workflow (Refactored with Clarification Pattern)

        NEW WORKFLOW (Multi-Turn Conversation):
        ...
        5. If missing: Set clarification_state and return (Supervisor handles user interaction)
        6. If complete: Apply defaults for optional fields and execute tool

        KEY PRINCIPLE: Agent NEVER talks to user. Only declares missing info via clarification_state.
        ...
        Returns:
            Updated state with:
                - clarification_state["fertilizer_agent"]: If fields missing (Supervisor asks user)
                - fertilizer_output: If fields complete (tool result)
        ...
        try:
            ...
            if clarification_req:
                # Missing required fields - set clarification_state and return
                logger.info(f"{self.name}: Missing fields {clarification_req['requested_fields']}, requesting clarification")

                # Initialize clarification_state if needed
                if state.get("clarification_state") is None:
                    state["clarification_state"] = {}

                # Set clarification request for this agent
                state["clarification_state"]["fertilizer_agent"] = clarification_req
            ...
            if "error" in result:
                ...
                state["fertilizer_output"] = result
            else:
                logger.info(
...
"""
        ...
        try:
            ...
            response, _ = await self.generation_service.generate_answer(
    ...
    def _normalize_hindi_params(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if params.get("crop_type"):
            ...
            elif not self._is_valid_english_crop(crop):
        ...
        if params.get("soil_type"):
            ...
            elif not self._is_valid_english_soil(soil):
    ...
    def _is_valid_english_soil(self, soil: str) -> bool:
        ...
        return soil.lower() in valid_soils
