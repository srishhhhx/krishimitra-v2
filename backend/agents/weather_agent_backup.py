...
class WeatherAgent(ToolAgent):
    ...
    async def run(self, state: AgentState) -> AgentState:
        """
        Execute weather retrieval workflow
        ...
        5. Update state with weather_output

        Args:
            state: Current agent state with user_query

        Returns:
            Updated state with weather_output containing:
                - temperature: Current temperature (°C)
                - humidity: Relative humidity (%)
                - conditions: Weather description
                - location: Resolved location name
                - agricultural_insights: Farming-relevant insights
        """
        try:
            ...
            location_info = await self._extract_location_with_llm(query)
            ...
            if not location_info or not location_info.get("location"):
                ...
                state["weather_output"] = {
                    ...
                    "question": "Which location would you like weather information for? (e.g., Bangalore, Delhi, Punjab)"
                }
            ...
            if not coordinates:
                state["weather_output"] = {
                    ...
                    "needs_clarification": True,
                    "question": f"I don't recognize '{location_name}'. Please specify a major Indian city or state."
                }
            ...
            if "error" in weather_result and weather_result["error"]:
                ...
                state["error"] = f"{self.name}: {weather_result['error']}"
                state["weather_output"] = weather_result
                return state
